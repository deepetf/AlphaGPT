...
class StrategyVerifier:
    ...
    def __init__(
        ...
        self.bundle_info: Optional[Dict] = None
    ...
    def _load_strategy(self):
        ...
        if self.bundle_path:
            if self.king_step is not None:
                raise ValueError("--bundle cannot be used together with --king")
            self._load_strategy_from_bundle()
            return
    ...
    def _load_strategy_from_bundle(self):
        bundle_info = load_strategy_bundle(self.bundle_path)
        self.bundle_info = bundle_info
        bundle = bundle_info["bundle"]
        params = bundle_info["params"]

        if self.strategy_id and bundle_info.get("strategy_id") and self.strategy_id != bundle_info["strategy_id"]:
            raise ValueError(
                f"--strategy-id={self.strategy_id} 与 bundle.strategy_id={bundle_info['strategy_id']} 不一致"
            )

        self.strategy_id = bundle_info.get("strategy_id") or self.strategy_id or "bundle_strategy"
        self.strategy_name = bundle_info.get("strategy_name") or self.strategy_id
        self.formula = bundle_info["formula"]
        self.formula_info = {
            "id": self.strategy_id,
            "name": self.strategy_name,
            "formula_summary": bundle_info.get("formula_summary", {}),
        }

        self.top_k = self.top_k if self.top_k is not None else int(params.get("top_k", RobustConfig.TOP_K))
        self.fee_rate = self.fee_rate if self.fee_rate is not None else float(params.get("fee_rate", RobustConfig.FEE_RATE))
        if self.take_profit_ratio is None:
            self.take_profit_ratio = float(params.get("take_profit_ratio", RobustConfig.TAKE_PROFIT))
        if self.initial_cash is None:
            self.initial_cash = float(params.get("initial_capital", 1_000_000.0))

        self.formula_source = bundle_info["bundle_path"]
        logger.info(
            "Using strategy bundle: id=%s, name=%s, top_k=%s, fee_rate=%s, tp=%s, initial_cash=%s",
            self.strategy_id,
            self.strategy_name,
            self.top_k,
            self.fee_rate,
        ...
        logger.info("Bundle formula path: %s", bundle_info["formula_path"])
        logger.info("Bundle model config: %s", bundle_info["model_config_path"])
        logger.info(f"Formula: {' '.join(self.formula)}")
        self._validate_formula_or_raise()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='strategy bundle path generated by workflow.bundle_builder; when set, verify runs a single bundled strategy'
    )
    parser.add_argument(
        '--strategy-id',
        type=str,
    ...
    if args.bundle:
        ...
        if args.strategy_id and bundle_info.get("strategy_id") and args.strategy_id != bundle_info["strategy_id"]:
            raise ValueError(
                f"--strategy-id={args.strategy_id} 与 bundle.strategy_id={bundle_info['strategy_id']} 不一致"
            )
        if effective_config:
            logger.info(
            ...
            effective_config = bundle_info["model_config_path"]
    ...
    if args.bundle:
        if args.king is not None:
            raise ValueError("--bundle cannot be used together with --king")
        if args.strategies_config:
            logger.info("--bundle is set; --strategies-config will be ignored")
        _run_one(bundle_info["strategy_id"] if bundle_info else None)
    elif args.king is not None:
